...
$this->title = 'Новый заказ ('.$order->id.') — '.Yii::$app->formatter->asDate( $order->created_at, 'dd.MM.yyyy');
...
<div>
    <div style="text-align:left;color:rgb(115,115,115);line-height:150%;font-family:'Helvetica Neue',Helvetica,Roboto,Arial,sans-serif;">
        <p style="margin:0px 0px 16px">Вы получили заказ от <?=$userInfo['name']?> <?=$userInfo['surname']?>. Детали заказа:</p>
       <!--  <p style="margin:0px 0px 16px">&nbsp;</p> -->
        <h2 style="margin:16px 0px 8px;text-align:left;color:rgb(85,125,161);line-height:130%;font-family:'Helvetica Neue',Helvetica,Roboto,Arial,sans-serif;font-size:18px;display:block;"><span style="text-decoration: underline;">Заказ #<?=$order->id; ?></span> <b>(<?=Yii::$app->formatter->asDate( $order->created_at, 'dd.MM.yyyy'); ?>)</b></h2>
    ...
    <h2 style="margin:16px 0px 8px;text-align:left;color:rgb(85,125,161);line-height:130%;font-family:'Helvetica Neue',Helvetica,Roboto,Arial,sans-serif;font-size:18px;font-weight:bold;display:block">Информация о клиенте</h2>
    <div>
