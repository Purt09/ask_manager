...
<?= GridView::widget([
    ...
    'columns' => [
        ...
        [
            ...
            'value' => function ($model) {
                if( !in_array($model->username,['admin','manager'])){
                    if ($model->isBlocked) {
            ...
            'buttons' => [
                'update' => function ($url, $model, $key) {
                    if (Yii::$app->user->identity->id == $model->id) {
                            return '
                        <a data-method="POST" href="' . Url::to(['update', 'id' => $model->id]) . '">
                        <span title="' . Yii::t('user', 'Редактировать') . '" class="glyphicon glyphicon-pencil">
                        </span> </a>';
                    }
                ...
                'delete' => function ($url, $model, $key) {
                    ...
                    if( !in_array($model->username,['admin','manager'])){
                        if (!$model->isAdmin) {
                            return '
                        <a data-method="POST" data-confirm="' . Yii::t('user', 'Вы уверены, что хотите удалить этот элемент?') . '" href="' . Url::to(['delete', 'id' => $model->id]) . '">
                        <span title="' . Yii::t('user', 'Удалить') . '" class="glyphicon glyphicon-trash">
                        </span> </a>';
                        }
                    } else {
                        return '';
